int chos;
int L1=30; //Length of Ll
int L2=15; //Length of L2
float Q1,Q2,m; // Q1 & Q2 are the angles to calculate 
float x,y,q1,q2;
int Q=50;
#include <Servo.h>
Servo motol; // create servo object to control a servo 
Servo moto2;
...
LiquidCrystal lcd(rs, en, d4, d5, d6, d7); // pins of lcd connect to arduino
   ...
   void setup(){
  motol.attach(3);  //attaches the servo on pin 1to the servo object
  moto2.attach(4);  //attaches the servo on pin 2to the servo object
  lcd.begin(16, 2);
...
void loop(){
 ...
 if (chos==1){
  ...
  while (g2==0) {g2=customKeypad.getKey();} //wait user to enter second number of Q1   g2=g2-48;
  g2=g2-48;
  lcd.print(g2); 
  delay(500);
  q1=g1*10+g2; // sum gl & g2
  lcd.clear();
  lcd.print("Enter anlgle AN2");
 ...
 invers() ; //call invers function
    ...
    int FI(){
 // this function to let the user choose type of control the arm forward/inverse kinema  char kp=0;
 char kp=0;
 motol.write(0); 
 moto2.write(0); 
 lcd.clear();
 ...
 if(kp==1){
  ...
  delay(2000); 
   ...
   void forward(){
 //this function to calculate 01 &Q2 motol.write(q1);
 motol.write(q1);
 moto2.write(q2); 
 lcd.clear();
 Q1=q1*PI/180;
 ...
 lcd.clear();
 m=(pow(x,2)+pow(y,2)-pow(L1,2)-pow(L2,2))/(2*L1*112);
 q2=acos(m);
 Q2=q2*180/PI;
 Q1=Q-Q2;
